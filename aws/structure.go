...
// always returns the primary ip as the first element of the array
...
func expandPrivateIPAddresses(ips []interface{}, primaryIP string) []*ec2.PrivateIpAddressSpecification {
	...
	for i, v := range ips {
		...
		if len(primaryIP) > 0 {
			new_private_ip.Primary = aws.Bool(v.(string) == primaryIP)
		} else {
